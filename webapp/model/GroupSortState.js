sap.ui.define([
	...
	return BaseObject.extend("ZAPP_APPR_BAL.ZAPP_APPR_BAL.model.GroupSortState", {
		 ...
		 * @alias ZAPP_APPR_BAL.ZAPP_APPR_BAL.model.GroupSortState
		 */
		...
		/**
		 * Groups by Amt, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Amt") {
				...
				this._oViewModel.setProperty("/sortBy", "Amt");

				aSorters.push(
					new Sorter("Amt", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "BalId");
			}
